...
def register():
	if(request.method == 'POST'):
		...
		uname_results = cur.fetchall()
		for i in range(0,len(uname_results)):
			if(uname_results[i][0] == username):
				cur.close()
				return render_template('register.html',the_message = 'UserName already registered')
	...
	if(search_param == 'ISBN'):
	    ...
	    for i in range(0,len(isbn_results)): 
	    	if(book_detail == isbn_results[i][0]):
	    	    cur.execute('SELECT * FROM Books WHERE ISBN = %s',(book_detail,))
	    ...
	    for i in range(0,len(author_results)): 
	    	if(book_detail.lower() == author_results[i][0].lower()):
	    	    cur.execute('SELECT * FROM Books WHERE Author = %s',(author_results[i][0],))
	    	    results = cur.fetchall()
	    	    cur.close()
	    	    return render_template('search.html',results = results)
	    	else:
	    		str2 = list(map(str,(author_results[i][0].lower()).split(' ')))
	    		for name1 in str1:
	    			for name2 in str2:
	    				if(name1 == name2):
	    					cur.execute('SELECT * FROM Books WHERE Author = %s',(author_results[i][0],))
	    ...
	    for i in range(0,len(title_results)): 
	    	if(book_detail.lower() == title_results[i][0].lower()):
	    	    cur.execute('SELECT * FROM Books WHERE Author = %s',(title_results[i][0],))
	    	    results = cur.fetchall()
	    	    cur.close()
	    	    return render_template('search.html',results = results)
	    	else:
	    		...
	    		for name1 in str1:
	    			for name2 in str2:
	    				if(name1 == name2):
	    					cur.execute('SELECT * FROM Books WHERE Title = %s',(title_results[i][0],))
	    					results = cur.fetchall()
	    					for r in results:
	    						res.append(r)
	cur.close()		
...
def myreviews():
	cur = mysql.connection.cursor()
	cur.execute('SELECT Review,Rating,ISBN FROM Reviews where Email = %s',(session['email'],))
...
def info(isbn):
	...
	if(request.method == 'POST'):
		cur = mysql.connection.cursor()
		cur.execute('SELECT * FROM Reviews WHERE Email = %s AND ISBN = %s',(session['email'],isbn))
	...
	cur.execute('SELECT Review,Rating,Email FROM Reviews WHERE ISBN = %s',(isbn,))
...
def edit(isbn):
	cur = mysql.connection.cursor()
	cur.execute('SELECT Review FROM Reviews WHERE ISBN = %s AND Email = %s',(isbn,session['email']))
	review = cur.fetchall()
	flash(review[0][0])
	return redirect(url_for('info',isbn = isbn))
...
def delrev(isbn):
	cur = mysql.connection.cursor()
	cur.execute('DELETE FROM Reviews WHERE Email = %s AND ISBN = %s',(session['email'],isbn))
	mysql.connection.commit()
	cur.close()
	return redirect(url_for('info',isbn = isbn))
